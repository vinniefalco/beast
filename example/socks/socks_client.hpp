...
namespace socks {
    ...
    typename net::async_result<Handler, void(error_code)>::return_type
    async_handshake(AsyncStream& stream, const std::string& hostname, unsigned short port,
        int version, const std::string& username, const std::string& password,
        bool use_hostname, Handler&& handler)
    {
